...
class Shell:
    ...
    def check(
        ...
        **kwargs,
        ...
        return (
            cls.run(command, log_file, strict, verbose, dry_run, stdin_str, retries=retries, timeout=timeout, **kwargs)
            == 0
        )
    ...
    def run(
        ...
        def _check_timeout(timeout, process) -> None:
            if not timeout:
                return
            time.sleep(timeout)
            print(f"WARNING: Timeout exceeded [{timeout}], sending SIGTERM to process group [{process.pid}]")
            try:
            ...
            try:
                with open(log_file, "w") as log_fp:
                    ...
                    if proc.returncode == 0:
                        ...
                        if verbose:
                            print(f"ERROR: command [{command}] failed, exit code: {proc.returncode}, retry: {retry}/{retries}")
            except Exception as e:
                if verbose:
                    print(f"ERROR: command failed, exception: {e}, retry: {retry}/{retries}")
                if proc:
                    proc.kill()

        # Handle strict mode (ensure process success or fail)
        if strict:
            assert proc and proc.returncode == 0, f"Command failed with return code {proc.returncode}"
